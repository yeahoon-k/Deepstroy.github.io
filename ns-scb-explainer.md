...
**What the agent is up against.** The hidden data-generating process described by a time-indexed SCM (i.e., temporal models) carries information from one time step to the next. It causes the reward disparity for each time step. We view non-stationarity as a shift in the reward distribution. The key question becomes: which past variables actually transmit useful information to future rewards?
...
A **reward distribution shift** occurs when the reward distribution changes across time, given the intervention history $I_{1:t-1}$:
\\[
P(Y_t \mid \operatoname{do}()
\\]
